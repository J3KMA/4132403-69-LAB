"""แสดงโครงสร้างของข้อมูลที่จะถูกส่งออกไปในเครือข่าย (encapsulation)

หมายเหตุสำคัญ:
โปรแกรมนี้ใช้ TCP socket ธรรมดา ซึ่งเราเขียนได้แค่ส่วน payload เท่านั้น
ส่วน TCP / IP / Ethernet header เป็นหน้าที่ของ OS kernel และการ์ดเครือข่าย
ค่าที่แสดงด้านล่างจึงเป็นการ "จำลองให้เห็นภาพ" ว่า payload ของเราถูกห่อหุ้ม
(encapsulate) ด้วย header ชั้นไหนบ้างก่อนออกจากเครื่อง โดยใช้ค่าจริงเท่าที่
ดึงได้จาก socket (IP, port, ขนาดข้อมูล) ส่วนค่าที่ดึงไม่ได้จะเป็นค่ามาตรฐาน

ถ้าต้องการเห็น header จริง ๆ ต้องใช้ raw socket หรือดักจับด้วย Wireshark
"""
